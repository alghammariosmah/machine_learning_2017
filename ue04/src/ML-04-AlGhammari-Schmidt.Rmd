...
faces$data_corr <- faces$data[,-foundCorIndexes]
```

```{r data_partitioining}
# split into training and test data
set.seed(1704)
indexes_train <- createDataPartition(faces$pers, p=0.75, list = F)
indexes_test <- (1:nrow(faces$data))[-indexes_train]

training <- faces$data_corr[indexes_train,]
testing <- faces$data_corr[indexes_test,]
```

```{r feature_selection}
rfeRes <- rfe(x = training, y = faces$pers[indexes_train], rfeControl = rfeControl(functions = rfFuncs, method = 'repeatedcv', repeats = 5)) # more repeats are better
rfeRes
